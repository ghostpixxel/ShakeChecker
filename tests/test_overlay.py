...
def test_alpha_sprite_gets_red_glow(qt_app):
    ov = Overlay(BALLS)
    ov.show_battle(164, "Noctowl", 10, 1, {"Poké Ball": 0.5}, alpha=True)
    assert ov._alpha_glow.isEnabled() is True  # red aura shown for an alpha
    assert ov._alpha_glow.blurRadius() > 0
    ov.show_battle(164, "Noctowl", 90, 1, {"Poké Ball": 0.5}, alpha=False)
    assert ov._alpha_glow.isEnabled() is False  # off for a normal encounter
